...
public class Especie {
    ...
    /*El siguiente metodo siguiente permite alterar una solucion mezclandolo con una especie, los genes representa la posicion de un arreglo y 
    variante es un valor que estara relacionado con el cambio en cuestion, en este caso se cambiara de lugares*/
    public ArrayList<Coordenada> reOrdenamiento(ArrayList<Coordenada> lstpuntos)
    {
        ...
        for(int c = 0; c < this.genes.size();)
        {
            ...
            if(br.getDistance(lstpuntos.get(semilla), lstpuntos.get(semilla+this.variantes.get(c))) < br.getDistance(lstpuntos.get(semilla), lstpuntos.get(semilla + 1)))
            {
