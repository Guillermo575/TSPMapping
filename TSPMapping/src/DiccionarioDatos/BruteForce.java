...
public class BruteForce 
{
    ...
    public ArrayList<Coordenada> start(ArrayList<Coordenada>lstcoordenadas, Coordenada destino, boolean omitirorigen) 
    {
        ...
        while (cerrar == true) 
        {
            ...
            {
                int adelante = p + 1;
                if (adelante >= nuevaruta.size()) 
                {
                    adelante = nuevaruta.size() - 1;
                }
//                if(md!=null && md.rutas!=null)totalpuntos+=md.rutas[nuevaruta.get(p)][nuevaruta.get(adelante)];else 
    ...
    public double getDistance(Coordenada a, Coordenada b)
    {
        ...
        switch(TipoRuta)
        {
            case "EUC_2D": retDistance = getEuclidianDistance(a, b); break;
            case "ATT": retDistance = getPseudoEuclidianDistance(a, b); break;
            case "GEO": retDistance = getGeographicalDistance(a, b); break;
            case "CEIL_2D": retDistance = getCeilingDistance(a, b); break;
            default: retDistance = getEuclidianDistance(a, b); break;
        }
        return retDistance;
    }
