...
public class Coordenada{
    ...
    public static ArrayList<Coordenada> clonar(ArrayList<Coordenada> lstpuntos){
       ...
       for(int l=0;l<lstpuntos.size();l++){nuevo.add(new Coordenada(lstpuntos.get(l).x,lstpuntos.get(l).y));}
       return nuevo;
    }
    ...
    public static ArrayList<Coordenada> randomList(int total,double min, double max){
        ArrayList<Coordenada> nuevo= new ArrayList<Coordenada>();
        for(int l=0;l<total;l++){nuevo.add(new Coordenada((double) (Math.random() * (max - min))+min,(double) (Math.random() * (max - min))+min));}
        return nuevo;
     }
}
